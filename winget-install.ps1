...
function Get-OSInfo {
    ...
    try {
        ...
        Write-Error "Ýþletim sistemi sürümü ayrýntýlarý alýnamýyor.Hata: $_"
        exit 1
    }
}
...
function CheckForUpdate {
    ...
    if ($Data.LatestVersion -gt $CurrentVersion) {
        ...
        Write-Output "Yayýnlanma tarihi: $($Data.PublishedDateTime)."
        Write-Output "En son sürümü þuradan indirebilirsiniz: https://github.com/$RepoOwner/$RepoName/releases"
...
function Get-WingetDownloadUrl {
    ...
    Write-Debug "$uri 'den bilgi alýnýyor"
    $releases = Invoke-RestMethod -uri $uri -Method Get -ErrorAction stop

    Write-Debug "Son sürümü alýnýyor..."
...
function Update-PathEnvironmentVariable {
    ...
    foreach ($Level in "Machine", "User") {
        ...
        if (!$path.Contains($NewPath)) {
            if ($DebugMode) {
                Write-Output "$Level için PATH deðiþkenine $NewPath ekleniyor..."
            } else {
                Write-Output "$Level için PATH deðiþkeni ekleniyor..."
            }
            ...
            if ($DebugMode) {
                Write-Output "$NewPath için PATH deðiþkeninde zaten mevcut $Level, atlanýyor."
            } else {
                Write-Output "için zaten mevcut olan PATH deðiþkeni $Level, atlanýyor."
            }
        }
    }
}
...
function Handle-Error {
    ...
    if ($ErrorRecord.Exception.Message -match '0x80073D06') {
        ...
        Write-Warning "Sorun deðil, devam ediliyor...."
    } elseif ($ErrorRecord.Exception.Message -match '0x80073CF0') {
        Write-Warning "Ayný sürüm zaten yüklü."
        Write-Warning "Sorun deðil, devam ediliyor...."
        ...
        Write-Warning "Deðiþtirilen kaynaklar kullanýmda. Windows Terminal / PowerShell / Komut Ýstemi'ni kapatmayý ve tekrar denemeyi deneyin."
        Write-Warning "Sorun devam ederse, bilgisayarýnýzý yeniden baþlatýn."
        return $ErrorRecord
    } elseif ($ErrorRecord.Exception.Message -match 'Uzak sunucuya baðlanýlamýyor') {
        Write-Warning "Gerekli dosyalarý indirmek için Ýnternet'e baðlanýlamýyor."
        Write-Warning "Komut dosyasýný tekrar çalýþtýrmayý deneyin ve internete baðlý olduðunuzdan emin olun."
        Write-Warning "Bazen nuget.org sunucusu kapalý olabilir, bu nedenle daha sonra tekrar denemeniz gerekebilir."
        return $ErrorRecord
    } elseif ($ErrorRecord.Exception.Message -match "Uzaktaki ad çözümlenemedi") {
        Write-Warning "Gerekli dosyalarý indirmek için Ýnternet'e baðlanýlamýyor."
        Write-Warning "Komut dosyasýný tekrar çalýþtýrmayý deneyin ve internete baðlý olduðunuzdan emin olun."
        Write-Warning "Bilgisayarýnýzda DNS'in doðru çalýþtýðýndan emin olun."
    } else {
...
function Install-Prerequisite {
    ...
    Write-Section "${arch} ${Name} Ýndiriliyor & kuruluyor..."
    ...
    try {
        ...
        $messageTemplate = "{OS} algýlandý. {NAME}'in {DOMAIN} sürümü kullanýlýyor."
        ...
        Write-Output "$Name baþarýyla kuruldu."
    } catch {
        # Alternate method
        ...
        try {
            ...
            if ($ThrowReason.Code -eq 0) {
                Write-Warning "$Name indirilmeye veya yüklenmeye çalýþýlýrken hata oluþtu. Alternatif yöntem deneniyor..."
            } else {
            ...
            if ($Name -eq "UI.Xaml") {
                ...
                Write-Output "UI.Xaml baþarýyla kuruldu."
            ...
            if ($_.Exception.Message -match "Uzak sunucuya baðlanýlamýyor") {
                ...
                if ($ShowOldVersionMessage) {
                    $OldVersionMessage = "$Name dosyasýný indirmek için sunucuya baðlanmada bir sorun var. Ne yazýk ki bu, önkoþul sunucu URL'leriyle ilgili bilinen bir sorundur - bazen kapalý olurlar. $WindowsCaption kullandýðýnýz için önkoþullarýn maðaza dýþý sürümlerini kullanmanýz gerekir, Windows maðazasýndaki önkoþullar çalýþmayacaktýr, bu nedenle daha sonra tekrar denemeniz veya manuel olarak yüklemeniz gerekebilir."
                    Write-Warning $OldVersionMessage
                } else {
                    Write-Warning "$Name indirmeye veya yüklemeye çalýþýrken hata oluþtu. Lütfen daha sonra tekrar deneyin veya $Name'i manuel olarak yükleyin."
                }
            }
...
if ($osVersion.Type -eq "Workstation" -and $osVersion.NumericVersion -lt 10) {
    Write-Error "winget yalnýzca Windows 10 veya üzeri sürümlerle uyumludur."
    exit 1
}
...
if ($osVersion.Type -eq "Workstation" -and $osVersion.NumericVersion -eq 10 -and $osVersion.ReleaseId -lt 1809) {
    Write-Error "winget yalnýzca Windows 10 sürüm 1809 veya üstü ile uyumludur."
    exit 1
}

# If it's a server, it needs to be 2022+
if ($osVersion.Type -eq "Server" -and $osVersion.NumericVersion -lt 2022) {
    Write-Error "winget yalnýzca Windows Server 2022+ ile uyumludur."
    exit 1
}
...
if (Get-WingetStatus) {
    if ($Force -eq $false) {
        Write-Output "winget zaten kurulu..."
        exit 0
    }
}
...
try {
    ...
    Write-Output "GitHub'dan winget için indirme URL'si alýnýyor..."
    ...
    try {
        ...
        Write-Output "winget baþarýyla kuruldu."
    } catch {
        $errorHandled = Handle-Error $_
        if ($null -ne $errorHandled) {
            throw $errorHandled
    ...
    Write-Section "Mevcut deðilse, geçerli kullanýcý için WindowsApps dizinini kontrol eder ve PATH deðiþkenine ekler..."
    ...
    Write-Section "Kurulum tamamlandý!"

    # Timeout for 5 seconds to check winget
    Write-Output "Winget'in kurulu ve çalýþýr durumda olup olmadýðýný kontrol ediliyor..."
    Start-Sleep -Seconds 3

    # Check if winget is installed
    if (Get-WingetStatus -eq $true) {
        Write-Output "winget þimdi kuruldu ve çalýþýyor, devam edebilir ve kullanabilirsiniz."
    } else {
        Write-Warning "winget yüklü ancak bir komut olarak algýlanmýyor. Þimdi winget'i kullanmayý deneyin. Çalýþmazsa, yaklaþýk 1 dakika bekleyin ve tekrar deneyin (bazen gecikebilir). Ayrýca bilgisayarýnýzý yeniden baþlatmayý deneyin."
        Write-Warning "Bilgisayarýnýzý yeniden baþlatýrsanýz ve komut hala tanýnmazsa, lütfen README: https://github.com/asheroto/winget-install#troubleshooting adresindeki Sorun Giderme bölümünü okuyun."
        Write-Warning "Bu komutu çalýþtýrarak betiðin en son sürümüne sahip olduðunuzdan emin olun: $PowerShellGalleryName -CheckForUpdate"
    }
} catch {
    # ============================================================================ #
    # Error handling
    ...
    Write-Section "UYARI! Kurulum sýrasýnda bir hata oluþtu!"
    Write-Warning "Yukarýdaki mesajlar yardýmcý olmaz ve sorun devam ederse, lütfen README: https://github.com/asheroto/winget-install#troubleshooting adresindeki Sorun Giderme bölümünü okuyun."
    Write-Warning "Bu komutu çalýþtýrarak betiðin en son sürümüne sahip olduðunuzdan emin olun: $PowerShellGalleryName -CheckForUpdate"
